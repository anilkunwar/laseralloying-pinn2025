...
class TemperaturePINN(nn.Module):
    def __init__(self, k, rho, C, Lx, Ly, Lz, T_max):
        ...
        # Initialize SmoothSigmoid scale
...
def evaluate_model(_model, times, Lx=100, Ly=100, Lz=100):
    n_points = 30  # Coarser grid for memory
    ...
    for t_val in times:
        ...
        if t_val == times[0]:
            boundary_checks['T_bottom'] = T[:, :, 0]  # z=0
            boundary_checks['T_top'] = T[:, :, -1]    # z=Lz
            st.write(f"T at z=0, x=0, y=0: {T[0,0,0]:.2f} K, Expected ≈ 400K")
            st.write(f"T at z=Lz, x=0, y=0: {T[0,0,-1]:.2f} K, Expected ≈ 300K")
    
    return X.numpy(), Y.numpy(), Z.numpy(), T_preds, boundary_checks

def create_plotly_plot(X, Y, Z, T_list, times, Lx, Ly, Lz):
    st.markdown("## 3D Temperature Dynamics Visualization")
    ...
    fig.update_layout(
        ...
        title=f"Temperature @ t={times[time_index]:.2f}s<br>Domain: {Lx:.0f}μm × {Ly:.0f}μm × {Lz:.0f}μm"
    )
    
    st.plotly_chart(fig, use_container_width=True)

if __name__ == "__main__":
    st.title("3D Temperature PINN Visualization")
